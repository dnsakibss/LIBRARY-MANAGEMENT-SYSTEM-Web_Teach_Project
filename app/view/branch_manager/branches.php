...
<?php if (empty($branches)): ?>
  <div class="alert alert-info text-center">
    <i class="bi bi-info-circle me-2"></i>No branches found. Click <strong>Add Branch</strong> to get started.
  </div>
<?php else: ?>
...
<div class="row g-3">
  ...
  <div class="col-md-4">
    <div class="card border-0 shadow-sm h-100 <?= !$b['is_active'] ? 'opacity-50' : '' ?>" title="<?= e($b['name']) ?> — <?= $b['is_active'] ? 'Active' : 'Inactive' ?>">
      <div class="card-body">
        ...
        <p class="text-muted small mb-0"><i class="bi bi-clock me-1"></i>Added: <?= e($b['created_at'] ?? 'N/A') ?></p>
      </div>
      <div class="card-footer bg-white d-flex gap-2">
        <a href="<?= BASE_URL ?>index.php?page=manager_branch_edit&id=<?= $b['id'] ?>" class="btn btn-sm btn-outline-warning flex-fill" title="Edit <?= e($b['name']) ?>">
        ...
        <form method="POST" class="flex-fill">
          ...
          <button class="btn btn-sm btn-outline-<?= $b['is_active'] ? 'secondary' : 'success' ?> w-100"
            ...
            title="<?= $b['is_active'] ? 'Deactivate' : 'Activate' ?> this branch">
