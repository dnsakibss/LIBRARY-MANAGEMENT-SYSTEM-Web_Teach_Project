...
<div class="d-flex justify-content-between align-items-center mb-4">
  ...
  <span class="badge bg-secondary fs-6" title="Total inventory records">
    <?= count($inventory) ?> record(s)
  </span>
</div>
      ...
      <div class="card-body">
        <form method="POST">
          <div class="mb-3">
            ...
            <select name="branch_id" class="form-select form-select-sm" required title="Select a branch to update">
          ...
          <div class="mb-3">
            ...
            <select name="book_id" class="form-select form-select-sm" required title="Select a book to update stock">
          ...
          <div class="row g-2 mb-3">
            <div class="col-6">
              ...
              <input type="number" name="total" class="form-control form-control-sm" min="1" value="1" required title="Total number of copies">
            </div>
            <div class="col-6">
              <label class="form-label small fw-semibold">Available Now</label>
              <input type="number" name="available" class="form-control form-control-sm" min="0" value="1" required title="Copies currently on shelf">
            </div>
          </div>
          ...
          <button class="btn btn-danger btn-sm w-100" title="Save inventory changes">
  ...
  <div class="col-md-8">
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white fw-semibold d-flex justify-content-between align-items-center">
        <span><i class="bi bi-table me-2 text-danger"></i>All Branches Inventory</span>
        <?php if (!empty($inventory)):
          $totalBorrowed = array_sum(array_map(fn($r) => $r['total_copies'] - $r['available_copies'], $inventory));
        ?>
        <span class="badge bg-warning text-dark" title="Total books currently borrowed">
          <i class="bi bi-book me-1"></i><?= $totalBorrowed ?> borrowed
        </span>
        <?php endif; ?>
      </div>
            ...
            <?php if (empty($inventory)): ?>
              <tr>
                <td colspan="7" class="text-muted text-center py-4">
                  <i class="bi bi-inbox me-2"></i>No inventory data found.
                </td>
              </tr>
            ...
            <tr>
              ...
              <td class="text-center">
                <span class="badge bg-<?= $row['available_copies'] > 0 ? 'success' : 'danger' ?> fs-6"
                  title="<?= $row['available_copies'] ?> copies available">
                  <?= $row['available_copies'] ?>
                </span>
              </td>
              <td class="text-center"><?= $row['total_copies'] ?></td>
              <td class="text-center">
                <?= $borrowed > 0 ? "<span class='badge bg-warning text-dark' title='$borrowed copies borrowed'>$borrowed</span>" : '<span class="text-muted">—</span>' ?>
              </td>
              <td style="min-width:80px">
                <div class="progress" style="height:6px" title="<?= round($pct) ?>% available">
