...
<div class="card border-0 shadow-sm">
  <div class="card-body p-0">
      ...
      <table class="table table-hover align-middle mb-0 small">
        ...
        <?php if (empty($books)): ?>
          <tr><td colspan="8" class="text-muted text-center py-5">
            ...
            No books found<?= $search ? " for \"" . e($search) . "\"" : '' ?>.
          </td></tr>
        <?php else: foreach ($books as $i => $b): ?>
        <tr>
          <td class="text-muted"><?= $i + 1 ?></td>
          <td>
            <?php if ($b['cover_image_path']): ?>
              <img src="<?= BASE_URL ?>public/<?= e($b['cover_image_path']) ?>"
                   height="45" style="object-fit:cover;border-radius:4px;box-shadow:0 1px 4px rgba(0,0,0,.15)">
            <?php else: ?>
              <div class="bg-light rounded d-flex align-items-center justify-content-center"
                   style="width:32px;height:45px"><i class="bi bi-image text-muted small"></i></div>
            <?php endif; ?>
          </td>
          ...
          <td>
            <span class="badge bg-secondary"><?= e($b['genre_name'] ?? '—') ?></span>
          </td>
          <td><?= e($b['published_year'] ?? '—') ?></td>
          <td class="text-center">
            <div class="d-flex gap-1 justify-content-center">
              <a href="<?= BASE_URL ?>index.php?page=admin_book_edit&id=<?= $b['id'] ?>"
                 class="btn btn-sm btn-outline-warning" title="Edit Book">
                <i class="bi bi-pencil"></i>
              </a>
              <form method="POST" class="d-inline"
                    onsubmit="return confirm('Permanently delete \'<?= e(addslashes($b['title'])) ?>\'? This cannot be undone.')">
    ...
    <div class="card-footer bg-white text-muted small">
      ...
      <?php if ($search): ?> — filtered by "<?= e($search) ?>"<?php endif; ?>
    </div>
  </div>
</div>
