...
<h2 class="mb-4"><i class="bi bi-arrow-left-right me-2 text-success"></i>Inter-Branch Transfers</h2>
<div class="row g-4">
  <div class="col-md-4">
    <div class="card border-0 shadow-sm">
      <div class="card-body">
        ...
        <form method="POST">
          ...
          <button class="btn btn-success btn-sm w-100"><i class="bi bi-send me-1"></i>Request Transfer</button>
        </form>
      </div>
    </div>
  </div>
  <div class="col-md-8">
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white fw-semibold">Transfer Requests Involving This Branch</div>
      <div class="card-body p-0">
        <?php if (empty($transfers)): ?>
          <p class="text-muted text-center py-4">No transfer requests.</p>
        <?php else: ?>
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0 small">
            <thead class="table-light">
              <tr><th>Book</th><th>From</th><th>To</th><th>Status</th><th>Action</th></tr>
            </thead>
            <tbody>
            ...
            <tr>
              ...
              <td><?= e($t['to_branch']) ?></td>
              <td>
                <?php $badgeMap = ['pending'=>'warning','approved'=>'success','rejected'=>'danger','completed'=>'secondary'];
                $cls = $badgeMap[$t['status']] ?? 'secondary'; ?>
                <span class="badge bg-<?= $cls ?> text-<?= $cls==='warning'?'dark':'white' ?>"><?= ucfirst($t['status']) ?></span>
              </td>
              <td>
                ...
                <form method="POST" class="d-inline">
                  <input type="hidden" name="action" value="update">
                  <input type="hidden" name="transfer_id" value="<?= $t['id'] ?>">
                  <button name="status" value="completed" class="btn btn-xs btn-sm btn-info">Mark Complete</button>
                </form>
